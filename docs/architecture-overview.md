...
- **Telemetry Interpreter Agent** – Runs every telemetry cycle (or every five minutes) with the instructions in `infra/n8n/agents/telemetry-interpreter.md`. It inspects the last ~20 readings for room and battery temperatures, predicts near-term trends, applies failsafe commands (battery heater, load shed, heater scene changes), and emails operators when LiFePO₄ cells or occupied rooms approach unsafe ranges.
- **Daily Operations Summarizer Agent** – Uses `infra/n8n/agents/daily-summarizer.md` to produce a morning briefing. It rolls up min/max telemetry, automation actions, and unresolved alarms into a markdown digest that n8n emails to the ops team.
- **Command Reliability Auditor Agent** – Described in `infra/n8n/agents/command-auditor.md`. It scans the `commands` table hourly, resubmits stuck commands with bumped priority/redundancy keys, cancels conflicts, and escalates repeated failures to humans.

Each agent uses n8n's memory nodes to avoid oscillations and is wired to Postgres read/write nodes so they can both observe telemetry and push commands without leaving the existing workflow stack.
